...
void Bootloader::Connection(void) {
    ...
    if (currStatus != comm->isConnected()) {
        ...
        if (comm->isConnected()) {
            ...
            log("Connecting...");
            if(writeConfig) {
                //log("Disabling Erase button to prevent accidental erasing of the configuration words without reprogramming them\n");
                writeConfig = true;
                result = comm->LockUnlockConfig(false);
                if (result == Comm::Success) {
                    log("Unlocked Configuration bits successfully\n");
                }
            } else {
                writeConfig = false;
            }
...
void Bootloader::EraseDevice(void)
{
    ...
    {
        IoWithDeviceStarted("Erasing Device... (no status update until complete, may take several seconds)");
        elapsed.start();

        result = comm->Erase();
        if(result != Comm::Success)
        {
            IoWithDeviceCompleted("Erase", result, ((double)elapsed.elapsed()) / 1000);
            return;
        }
...
void Bootloader::VerifyDevice()
{
    ...
    for (int k = 0; k < deviceData->ranges.size(); k++)
    {
        ...
        {
            elapsed.start();
            //IoWithDeviceStarted("Verifying Device's Program Memory...");
            ...
            if(result != Comm::Success)
            {
                ...
                //IoWithDeviceCompleted("Verifying Device's Program Memory", result, ((double)elapsed.elapsed()) / 1000);
            }
    ...
    for (int k = 0; k < deviceData->ranges.size(); k++)
    {
        ...
        if(writeFlash && (deviceRange.type == PROGRAM_MEMORY))
        {
            ...
            if(result != Comm::Success)
            {
                ...
                IoWithDeviceCompleted("Blank Checking Program Memory", result, ((double)elapsed.elapsed()) / 1000);
                return;
            }
            ...
            for(unsigned int i = 0; i < ((deviceRange.end - deviceRange.start) * device->bytesPerAddressFLASH); i++)
            {
                if((deviceRange.pDataBuffer[i] != 0xFF) && !((device->family == Device::PIC24) && ((i % 4) == 3)))
                {
                    ...
                    IoWithDeviceCompleted("Blank Check", Comm::Fail, ((double)elapsed.elapsed()) / 1000);
                    return;
                }
            }
            IoWithDeviceCompleted("Blank Checking Program Memory", Comm::Success, ((double)elapsed.elapsed()) / 1000);
        }
        else if(writeEeprom && deviceRange.type == EEPROM_MEMORY)
        {
            IoWithDeviceStarted("Blank Checking Device's EEPROM Memory...");
            ...
            {
                qWarning("Blank Check failed");
                IoWithDeviceCompleted("Blank Checking EEPROM Memory", result, ((double)elapsed.elapsed()) / 1000);
                return;
            }
            ...
            IoWithDeviceCompleted("Blank Checking EEPROM Memory", Comm::Success, ((double)elapsed.elapsed()) / 1000);
        }
        else
        {
            continue;
...
HexImporter::ErrorCode Bootloader::LoadFile(QString newFileName)
{
    QString msg;
    ...
    if(writeConfig && (import.hasConfigBits == false) && device->hasConfig())
    {
        ...
        {
            log("Unexpected internal error encountered.  Recommend restarting the application to avoid ""bricking"" the device.\n");
        }

        log(Warning, "This HEX file does not contain config bit information.\n\nAutomatically disabling config bit reprogramming to avoid leaving the device in a state that could prevent further bootloading.");
        writeConfig = false;
    }
...
void Bootloader::log(QString value) {
    emit message(Info, value);
}

void Bootloader::log(MessageType type, QString value) {
    emit message(type, value);
}
